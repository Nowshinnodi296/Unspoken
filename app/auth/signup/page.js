...
import { generatePseudonym, generateAvatarSeed } from '@/lib/pseudonyms'
...
export default function SignUp() {
  ...
  async function handleSignUp() {
    ...
    if (userId) {
      const { error: insertError } = await supabase.from('users').insert({ id: userId, pseudonym, avatar_seed: avatarSeed })
    ...
    <main style={{minHeight:'100vh', backgroundColor:'#0f0d14', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem 1.5rem', position:'relative', overflow:'hidden'}}>
      ...
      <div style={{position:'relative', zIndex:1, width:'100%', display:'flex', flexDirection:'column', alignItems:'center', animation:'fadeSlideUp 0.8s ease forwards'}}>
        ...
        <div style={{marginBottom:'2rem', textAlign:'center'}}>
          ...
          <p style={{fontSize:'0.7rem', color:'#4a4760', marginTop:'0.4rem', letterSpacing:'0.15em', textTransform:'uppercase'}}>join quietly</p>
        </div>
        ...
        <div style={{width:'100%', maxWidth:'380px', backgroundColor:'rgba(22,19,31,0.95)', border:'0.5px solid #2a2640', borderRadius:'18px', padding:'1.75rem', display:'flex', flexDirection:'column', gap:'1.25rem', backdropFilter:'blur(8px)'}}>
          ...
          <p style={{color:'#9b98b0', fontSize:'0.8rem', lineHeight:'1.6'}}>
            No real name. No photo. Just an email to keep your account safe.
          </p>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:'0.5rem'}}>
            ...
            <input type="password" placeholder="at least 6 characters" value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setFocusedInput('password')} onBlur={() => setFocusedInput(null)} style={inputStyle('password')} />
          </div>
          ...
          <p style={{fontSize:'0.75rem', color:'#4a4760', lineHeight:'1.6'}}>
            You will get a random name like <span style={{color:'#9b9be8'}}>quiet-moon-42</span> — no one will know who you are.
          </p>
          ...
          >
            {loading ? 'Creating your space...' : 'Join quietly'}
          </button>

          <a href="/auth/login" style={{textAlign:'center', fontSize:'0.75rem', color:'#4a4760', textDecoration:'none'}}>
