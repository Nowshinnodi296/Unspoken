  ...
  async function handleSignUp() {
    ...
    if (userId) {
      const { error: insertError } = await supabase
        .from('users')
        .insert({ id: userId, pseudonym, avatar_seed: avatarSeed })
      if (insertError) {
      ...
      <div style={{width:'100%', display:'flex', flexDirection:'column', alignItems:'center', animation:'fadeSlideUp 0.8s ease forwards'}}>
        ...
        <div style={{width:'100%', maxWidth:'380px', backgroundColor:'#16131f', border:'0.5px solid #2a2640', borderRadius:'18px', padding:'1.75rem', display:'flex', flexDirection:'column', gap:'1.25rem', animation:'fadeSlideUp 1s ease forwards'}}>
          ...
          <p style={{fontSize:'0.75rem', color:'#4a4760', lineHeight:'1.6'}}>
            ...
            <span style={{color:'#9b9be8'}}>quiet-moon-42</span> — no one will know who you are.
          </p>
          ...
          <a href="/auth/login" style={{textAlign:'center', fontSize:'0.75rem', color:'#4a4760', textDecoration:'none', transition:'color 0.2s'}}>
